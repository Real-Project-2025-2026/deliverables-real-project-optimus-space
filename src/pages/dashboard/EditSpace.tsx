...
import { Space, SpaceCategory, Amenity, UsageType, CancellationPolicy } from '@/types';
...
export default function EditSpace() {
  ...
  const { data: space, isLoading, isError } = useQuery({
    queryKey: ['space', id],
    queryFn: () => spaceService.fetchById(id!),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (space) {
      setFormData({
        ...
        allowedUsageTypes: space.allowedUsageTypes,
        minRentalDays: space.minRentalDays,
        maxRentalDays: space.maxRentalDays,
        depositRequired: space.depositRequired,
        depositAmount: space.depositAmount,
        cancellationPolicy: space.cancellationPolicy,
        instantBooking: space.instantBooking,
      });
      setImages(space.images.filter(img => img !== '/placeholder.svg'));
    }
  }, [space]);
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      if (!id) throw new Error('Space ID fehlt');

      // First update space data
      ...
      if (newImages.length > 0) {
        ...
        for (const file of newImages) {
          try {
            const url = await spaceService.uploadImage(id, file);
            // Base64 URLs don't need cache-busting, only external URLs do
            if (url.startsWith('data:')) {
              uploadedUrls.push(url);
            } else {
              uploadedUrls.push(`${url}?t=${Date.now()}`);
            }
          } catch (err) {
        ...
        // Update images array (combine existing + new)
        const allImages = [...images, ...uploadedUrls];
        await spaceService.updateImages(id, allImages.length > 0 ? allImages : ['/placeholder.svg']);
        setIsUploading(false);
      } else {
        // Update images if any were deleted or changed
        await spaceService.updateImages(id, images.length > 0 ? images : ['/placeholder.svg']);
      }
    },
    onSuccess: () => {
      ...
      toast({
        title: 'Gespeichert',
        description: 'Die Fläche wurde erfolgreich aktualisiert.',
      });
      setHasChanges(false);
      setNewImages([]);
      setNewImagePreviews([]);
    ...
    onError: (error: Error) => {
      toast({
        title: 'Fehler',
        description: error.message || 'Die Fläche konnte nicht gespeichert werden.',
        variant: 'destructive',
      });
    },
  });
  ...
  const handleChange = (field: keyof Space, value: unknown) => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > availableSlots) {
      toast({
        title: 'Hinweis',
        description: `Es wurden nur ${availableSlots} von ${files.length} Bildern hinzugefügt (Limit: ${MAX_IMAGES}).`,
      });
    }
          ...
          <div className="container max-w-4xl">
            <Card className="p-8 text-center">
              ...
              <h2 className="text-xl font-semibold text-foreground mb-2">
                Fläche nicht gefunden
              </h2>
              <p className="text-muted-foreground mb-4">
                Die angeforderte Fläche konnte nicht geladen werden.
              </p>
              <Button asChild>
                <Link to="/dashboard/landlord">Zurück zum Dashboard</Link>
              </Button>
            </Card>
          </div>
        </main>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">
                  Aktualisieren Sie die Details und Bilder Ihrer Fläche.
                </p>
              </div>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  {images.map((img, index) => {
                    ...
                    const imgSrc = img.startsWith('data:') ? img : (img.includes('?') ? img : `${img}?t=${Date.now()}`);
                    return (
                  ...
                  {newImagePreviews.map((preview, index) => (
                    <div key={`new-${index}`} className="relative group">
                      <img
                        ...
                        className="w-full h-32 object-cover rounded-lg border-2 border-primary border-dashed"
                      />
                      <button
                        type="button"
                      ...
                      <Badge className="absolute bottom-2 left-2" variant="pending">
                        Neu
                      </Badge>
                    </div>
                  ))}
                ...
                <p className="text-xs text-muted-foreground">
                  JPG, PNG oder WebP. Max. 10 MB pro Bild. Max. {MAX_IMAGES} Bilder pro Fläche ({totalImages}/{MAX_IMAGES} verwendet). Das erste Bild wird als Hauptbild verwendet.
                </p>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <label htmlFor="instantBooking" className="text-sm font-medium">
                    Sofortbuchung aktivieren (keine Bestätigung erforderlich)
                  </label>
                </div>
              </CardContent>
            </Card>
